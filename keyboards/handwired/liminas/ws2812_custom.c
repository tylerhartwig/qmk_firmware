...
// This should be driven by config later
static bool is_rgbw_pixel(int index) {
    return (index >= 30 && index < 124) || index >= 124 + 30;
}
...
// Assume all LEDs are RGBW, could be refined with config later
// use uint8 for mixing RGB and RGBW
static uint8_t                WS2812_BYTE_BUFFER[WS2812_LED_COUNT * 4 + 4];
...
void ws2812_init(void) {
    ...
    RP_DMA_MODE_WS2812 = DMA_CTRL_TRIG_INCR_READ |
                         ...
                         DMA_CTRL_TRIG_BSWAP | // <-- we're doing BSWAP since RP2040 is little endian
...
#if defined(WS2812_RGBW)
    if (is_rgbw_pixel(index)) { ws2812_rgb_to_rgbw(&ws2812_leds[index]); }
#endif
}
...
//    uprintf("custom flush LED count is %d", WS2812_LED_COUNT);

    sync_ws2812_transfer();

    int buf_idx = 0;
    for (int i = 0; i < WS2812_LED_COUNT; i++) {
        WS2812_BYTE_BUFFER[buf_idx++] = ws2812_leds[i].g;
        WS2812_BYTE_BUFFER[buf_idx++] = ws2812_leds[i].r;
        WS2812_BYTE_BUFFER[buf_idx++] = ws2812_leds[i].b;

        if (is_rgbw_pixel(i)) {
            //uprintf("ws2812_flush rgbw pixel %d", i);
            WS2812_BYTE_BUFFER[buf_idx++] = ws2812_leds[i].w;
        }
    }

    // Zero out rest of word
    while (buf_idx % 4 != 0) {
        WS2812_BYTE_BUFFER[buf_idx++] = 0;
    }

    // this can be derived from config probably
    uint32_t word_count = buf_idx / 4;

    dmaChannelSetSourceX(dma_channel, (uint32_t)WS2812_BYTE_BUFFER);
    dmaChannelSetCounterX(dma_channel, word_count);
